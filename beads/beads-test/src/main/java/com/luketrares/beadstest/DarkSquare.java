...
public class DarkSquare extends UGen {
	...
	public DarkSquare(AudioContext audioContext) {
...
//		}
	...
	private float darkBellSound(double pos) {
		...
		for (int j = 0; j < detailCount; j++) {
			...
			double angle = twoPi * (v1 / (double) (v4 != 0 ? v4 : 1)) * 30.0 * (pos); // + 50.0*noise();

			double a3val = Math.max(0.1, (1.0 + sqsin(angle3)) / 2.0);

			// double angle = 2.0 * Math.PI * (parameters[0]*(j+1)) *
			// (ellapsedTime + i * tinc);
			double jvalue = Math.sin(angle) * sqsin(angle2) * a3val * parameters[200 + j] / v0;
	...
	private double sqsin(double angle) {
